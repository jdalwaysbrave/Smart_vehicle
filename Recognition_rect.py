...
class handle(Thread):
    ...
    def run(self):
        num = self.args[0]
        if num == 0:
            s.send(b'\xff\x02\x01\x10\xff')
            s.send(b'\xff\x02\x02\x15\xff')
            print('Speed:15\n')
            time.sleep(1)
        if num == 1:
            s.send(b'\xff\x02\x01\x50\xff')
            s.send(b'\xff\x02\x02\x55\xff')
            print('Speed:30\n')
            time.sleep(1)
        if num == 2:
